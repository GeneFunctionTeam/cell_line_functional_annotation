...
my ($help, $bone_mut_data, $mut_freqs, $genes, $cell_lines, $output_genes_list, $mut_cons_classes, $output);

GetOptions (
  "bone_muts=s" => \$bone_mut_data,
...
$output = "bone_mutation_data_table" unless defined $output;
...
# ======================================================== #
# parse the BONE exome data file and extract mutation info
# ======================================================== #
...
my %bone_truncs;				# store truncating counts for cell*gene
my %bone_rec_mis;				# store recurrent missense counts for cell*gene
my %bone_other;					# store other counts for cell*gene

if(defined $bone_mut_data){
	# Open and read mutations file
	open BONE, "< $bone_mut_data" or die "Can't read mutations file $bone_mut_data: $!\n";
	my $header = <BONE>;
	while(<BONE>){
	  my @fields = split(/\t/);
	  my $entrez_gene = $fields[10];  						# this is actually a gene symbol
	  my $var_class = lc($fields[17]);						# [16]  Mutation Description - mix of upped and lower case chars used!
	  chomp($var_class);
	  my $sample = $fields[1];								# [1]   Sample name
	  my $genome_change = $fields[4] . "_" . $fields[5];	# [4]  Mutation GRCh37 genome position
	  my $prot_change = $fields[15];						# [14]  Mutation AA
#	  my $mutation_zygosity = $fields[15];					# [15]  Mutation zygosity - nota available
	  ...
	  my $bone_key = "$standard_cell_line\t$standard_gene";
	  ...
	  if($mutation_consequences{$var_class} eq "trunc"){	# if mutation is truncating
		$bone_truncs{$bone_key} ++;							# increment cellline+gene truncating count
	  }
	  ...
	  if(exists $details_table{$bone_key}{"bone_mut"}){
		$details_table{$bone_key}{"bone_mut"} .=  "; $prot_change";
		$details_table{$bone_key}{"bone_type"} .=  "; $mutation_consequences{$var_class}";
		if(exists($mut_freqs{$davoli_key})){
		  $details_table{$bone_key}{"bone_davoli_freq"} .= "; $mut_freqs{$davoli_key}";
		}
		else{
		  $details_table{$bone_key}{"bone_davoli_freq"} .= "; NA";
		}
	  }
	  else{
		$details_table{$bone_key}{"bone_mut"} =  $prot_change;
...
while(my ($seen_cell_line, $cell_line_seen_in_dataset) = each  %master_cell_lines_seen){
  ...
  foreach my $seen_gene (@genes_seen){
    ...
    if(exists($bone_truncs{$hash_key})){
      $trunc = 1;
    }
    
    if(exists($bone_rec_mis{$hash_key})){
      $rec_mis = 1;
    }
    
    if(exists($bone_other{$hash_key})){
      $other = 1;
    }
...
# bone_type
  if(exists $details_table{$details_key}{"bone_type"}){
    chomp($details_table{$details_key}{"bone_type"});
    print DETAILS "\t$details_table{$details_key}{'bone_type'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
# bone_mut
  if(exists $details_table{$details_key}{"bone_mut"}){
    chomp($details_table{$details_key}{'bone_mut'});
    print DETAILS "\t$details_table{$details_key}{'bone_mut'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
# bone_davoli_freq
  if(exists $details_table{$details_key}{"bone_davoli_freq"}){
    chomp($details_table{$details_key}{"bone_davoli_freq"});
    print DETAILS "\t$details_table{$details_key}{'bone_davoli_freq'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
Usage:
perl process_bone_exome_data.pl [options]

Options:
  --help                    Display this message and quit
  --bone_muts               Path to the bone mutations file [required]
