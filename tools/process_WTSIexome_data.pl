...
my ($help, $wtsi_exome_data, $mut_freqs, $genes, $cell_lines, $output_genes_list, $mut_cons_classes, $output);

GetOptions (
  "wtsi_exome_data=s" => \$wtsi_exome_data,
...
# ============================== #
# Process the WTSI mutation data #
# ============================== #
...
if(defined $wtsi_exome_data){
	# Open and read mutations file
	open WTSI, "< $wtsi_exome_data" or die "Can't read mutations file $wtsi_exome_data: $!\n";
...
while(my ($seen_cell_line, $cell_line_seen_in_dataset) = each  %master_cell_lines_seen){
  ...
  foreach my $seen_gene (@genes_seen){
    ...
    if(exists($wtsi_truncs{$hash_key})){
      $trunc = 1;
    }
    
    if(exists($wtsi_rec_mis{$hash_key})){
      $rec_mis = 1;
    }
    
    if(exists($wtsi_other{$hash_key})){
      $other = 1;
    }
...
Options:
  ...
  --wtsi_exome_data         Path to the WTSI mutations file [required]
